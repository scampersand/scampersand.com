# $Id: bashrc 4871 2013-04-10 01:43:20Z aron $
#
# .bashrc
...
setup_TERM() {
  ...
  case $TERM in
    *[-+]256color|rxvt*)
    ...
    screen*)
      terms=(
        # screen-256color-bce-s doesn't seem to work with weechat.
...
# Screen sessions can't use a forwarded agent once the original ssh connection
# dies.  That makes screen a lot less useful.  If keychain has info about
# a local ssh-agent running, then try to use that from screen sessions.
if [[ $TERM == screen* ]]; then
  kf=~/.keychain/$HOSTNAME-sh
  [[ -r $kf ]] || kf=~/.keychain/${HOSTNAME%%.*}-sh
  if ( source "$kf" && ssh-add -l ) &>/dev/null; then
    source "$kf" >/dev/null
  fi
fi
